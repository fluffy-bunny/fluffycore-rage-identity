...
console.log("≡ƒö╡ webauthn.js: Script started loading...");
...
async function LoginUser(returnFailedUrl, useConditionalUI = false, errorCallback = null, successCallback = null) {
  ...
  try {
    ...
    console.log("≡ƒöì LoginUser - Server response:", optionsJSON);
    console.log("≡ƒöì LoginUser - rpId:", optionsJSON.publicKey?.rpId || optionsJSON.rpId);
    console.log("≡ƒöì LoginUser - allowCredentials:", optionsJSON.publicKey?.allowCredentials || optionsJSON.allowCredentials);
    console.log("≡ƒöì LoginUser - Current hostname:", window.location.hostname);
    console.log("≡ƒöì LoginUser - Current origin:", window.location.origin);
    ...
    if (PublicKeyCredential.parseRequestOptionsFromJSON) {
      ...
      console.log("≡ƒöì LoginUser - Parsed options:", parsedOptions);
    ...
    console.log("≡ƒöì LoginUser - Final credentialRequestOptions being sent to navigator.credentials.get:", credentialRequestOptions);

    // Get credential
    console.log("≡ƒöì LoginUser - Calling navigator.credentials.get...");
    const credential = await navigator.credentials.get(credentialRequestOptions);
    
    console.log("≡ƒöì LoginUser - Received credential:", credential);
...
async function registerPasskey(friendlyName) {
  console.log("≡ƒƒó registerPasskey() called with friendlyName:", friendlyName);
  console.log("≡ƒƒó Function type:", typeof registerPasskey);
  console.log("≡ƒƒó Window object has registerPasskey:", "registerPasskey" in window);
  
  let abortController = null;
  
  try {
    console.log("≡ƒö╡ Step 1: Fetching registration options from /webauthn/register/begin");
    // Fetch registration options from server
    const response = await fetch("/webauthn/register/begin");

    if (!response.ok) {
      const errorText = await response.text();
      console.error("Γ¥î Server error:", response.status, errorText);
      throw new Error(`Server error: ${response.status} - ${errorText}`);
    }
    
    const optionsJSON = await response.json();
    console.log("Γ£à Step 1 complete: Registration options received:", optionsJSON);
    console.log("≡ƒöì optionsJSON structure:", JSON.stringify(optionsJSON, null, 2));
    console.log("≡ƒöì optionsJSON.publicKey:", optionsJSON.publicKey);
    console.log("≡ƒöì optionsJSON.publicKey.challenge:", optionsJSON.publicKey?.challenge);

    console.log("≡ƒö╡ Step 2: Parsing credential creation options");
    // Use parseCreationOptionsFromJSON if available (recommended by Google/W3C)
    let options;
    if (PublicKeyCredential.parseCreationOptionsFromJSON) {
    ...
    console.log("Γ£à Step 2 complete: Options parsed successfully");

    console.log("≡ƒö╡ Step 3: Creating credential (browser will show dialog)");
    // Create abort controller for timeout
    abortController = new AbortController();
    const timeoutId = setTimeout(() => {
      console.log("ΓÅ▒∩╕Å Timeout reached, aborting...");
      abortController.abort();
    }, 120000); // 2 minute timeout
    ...
    if (!credentialsResponse) {
      console.error("Γ¥î Failed to create credential - no response");
      throw new Error("Failed to create credential");
    }

    console.log("Γ£à Step 3 complete: Credential created:", credentialsResponse);

    console.log("≡ƒö╡ Step 4: Preparing credential data for server");
    ...
    if (!finishResponse.ok) {
      const errorData = await finishResponse.json().catch(() => ({}));
      console.error("Γ¥î Server rejected credential:", finishResponse.status, errorData);
      throw new Error(errorData.error || `Server error: ${finishResponse.status}`);
    }

    const data = await finishResponse.json();
    console.log("Γ£à Step 5 complete: Server accepted credential:", data);
    console.log("Γ£àΓ£àΓ£à PASSKEY REGISTRATION SUCCESSFUL! Γ£àΓ£àΓ£à");
    
    // Return success
    return true;
  } catch (error) {
    console.error("Γ¥îΓ¥îΓ¥î PASSKEY REGISTRATION FAILED:", error);
    ...
    // Handle InvalidStateError specially (passkey already exists)
    if (error.name === "InvalidStateError") {
      console.warn("ΓÜá∩╕Å Passkey already exists on this device");
...
console.log("≡ƒö╡ webauthn.js: registerPasskey function defined");
console.log("≡ƒö╡ webauthn.js: typeof registerPasskey =", typeof registerPasskey);
...
console.log("≡ƒö╡ webauthn.js: Assigning functions to window object...");
window.registerPasskey = registerPasskey;
window.registerUser = registerUser;
window.LoginUser = LoginUser;

// Log when script is fully loaded
console.log("Γ£àΓ£àΓ£à webauthn.js: SCRIPT FULLY LOADED Γ£àΓ£àΓ£à");
console.log("Γ£à All functions registered on window object:");
...
console.log("Γ£à Script ready - WASM can now call these functions");
...
document.addEventListener("DOMContentLoaded", function () {
  if (!window.PublicKeyCredential) {
    ...
    console.log("Γ£à WebAuthn supported in this browser");
  }
});
