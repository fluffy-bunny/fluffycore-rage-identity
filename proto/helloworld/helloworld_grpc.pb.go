...
type MyStreamServiceClient interface {
	...
	// Request: Unary
	// Response: Streaming
	RequestPoints(ctx context.Context, in *PointsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Point], error)
	// Accepts a stream of Points on a route being traversed, returning a
	// RouteSummary when traversal is completed.
	StreamPoints(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[Point, RouteSummary], error)
}
...
func (c *myStreamServiceClient) RequestPoints(ctx context.Context, in *PointsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Point], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &MyStreamService_ServiceDesc.Streams[0], MyStreamService_RequestPoints_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[PointsRequest, Point]{ClientStream: stream}
...
func (c *myStreamServiceClient) StreamPoints(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[Point, RouteSummary], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type MyStreamServiceServer interface {
	...
	// Request: Unary
	// Response: Streaming
	RequestPoints(*PointsRequest, grpc.ServerStreamingServer[Point]) error
	// Accepts a stream of Points on a route being traversed, returning a
	// RouteSummary when traversal is completed.
	StreamPoints(grpc.ClientStreamingServer[Point, RouteSummary]) error
	mustEmbedUnimplementedMyStreamServiceServer()
}

// UnimplementedMyStreamServiceServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
...
func (UnimplementedMyStreamServiceServer) testEmbeddedByValue()                         {}
...
func RegisterMyStreamServiceServer(s grpc.ServiceRegistrar, srv MyStreamServiceServer) {
	// If the following call pancis, it indicates UnimplementedMyStreamServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
...
func _MyStreamService_RequestPoints_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(MyStreamServiceServer).RequestPoints(m, &grpc.GenericServerStream[PointsRequest, Point]{ServerStream: stream})
}
...
func _MyStreamService_StreamPoints_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(MyStreamServiceServer).StreamPoints(&grpc.GenericServerStream[Point, RouteSummary]{ServerStream: stream})
}
