...
func (s *service) DoGet(c *echo.Context) error {
	...
	// Also delete management session cookie to ensure we're logged out from management
	wellknownCookieNames := s.WellknownCookieNames()
	if wellknownCookieNames != nil {
		...
		if managementSessionCookieName != "" {
			s.wellknownCookies.DeleteInsecureCookie(c, managementSessionCookieName)
		}
	}

	// Redirect immediately without showing a page (to avoid creating a new session via /api/login)
