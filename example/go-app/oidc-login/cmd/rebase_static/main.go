...
func main() {
	...
	oldBase := flag.String("old-base", "/oidc-login", "Old base href path")
	...
	if *inputDir == "" || *outputDir == "" || *oldBase == "" || *newBase == "" {
		...
		fmt.Println("Example: rebase_static -input=./static_output -output=./rebased_output -old-base=/oidc-login -new-base=/my/nested/app")
		os.Exit(1)
	}
...
func processTextFile(inputPath, outputPath, oldBase, newBase string) error {
	...
	// Match patterns like /oidc-login/ and /oidc-login
