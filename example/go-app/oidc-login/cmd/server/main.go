...
func main() {
	...
	log.Info().
		Msg("Starting selfhost application")

	// Parse command line arguments
	staticDir := flag.String("dir", "./static_output", "Directory containing static files")
	basePath := flag.String("base", "/oidc-login", "Base path for serving the app")
	...
	*basePath = "/" + strings.Trim(*basePath, "/")
	...
	e.GET("/api/app-config", appConfigHandler)
...
<body>
    <div class="container">
        ...
        <p>Click below to access the login flow</p>
        <a href="%s/" class="btn">Continue to Login</a>
    </div>
</body>
</html>`, *basePath)
...
func appConfigHandler(c echo.Context) error {
	...
	log.Info().Interface("config", config).Msg("Serving config")
